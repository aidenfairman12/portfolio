...
const CLIMATE_QA = {
  ...
  type:        'Solo project',
  status:      'Research' as const,
  description: 'A multiple-choice benchmark for evaluating how well large language models reason about ' +
               'climate science, grounded in IPCC AR6 source passages across all three Working Groups. ' +
               '580 candidate questions were generated by four frontier models (Claude, GPT-4o, Gemini, ' +
  ...
  highlights: [
    ...
    { icon: Columns2,              label: 'Streamlit curation UI',     desc: 'Side-by-side 4-model review interface with batch navigation and auto-save' },
    { icon: BarChart,             label: 'Self vs cross-model eval',   desc: 'Claude +9.1pp self-advantage; GPT-4o and Llama score lower on own questions' },
  ],
}

const CLIMATE_QA_CASE_STUDY = {
  intro: [
  ...
  decisions: [
    {
      title: 'Four generator models rather than one',
      body: 'Using Claude, GPT-4o, Gemini, and Llama as generators served two purposes: it increases question diversity (different models surface different aspects of the same passage), and it enables a later analysis of whether models perform better on questions they themselves generated. That self vs cross-model analysis turned out to be one of the most interesting findings in the project.',
    },
    {
      title: 'Cold evaluation: no source passage provided',
      body: 'Each model was evaluated with only the question and answer options, no source passage. This tests whether models have internalised the relevant climate science from training rather than extracting an answer from provided text. The tradeoff is that some questions hinge on specific IPCC AR6 values that a model could only know from training data — a minor confound worth noting, since all four models were trained after AR6\'s 2021–2022 publication.',
    },
    {
      title: 'Streamlit curation over a scripted filter',
      body: 'With 580 candidate questions across 29 batches and 4 models, a systematic scripted filter would have needed explicit quality criteria that are hard to define upfront. A side-by-side Streamlit interface with the source passage collapsible alongside each question made it practical to identify duplicate questions across models and compare distractor quality directly. The review was intentionally relaxed — the goal was removing clearly broken questions rather than enforcing a uniform quality bar.',
    },
  ],
  nextSteps: [
    {
      title: 'Better passage selection upstream',
      body: 'Thirteen of 29 batches produced five or fewer accepted questions. Attribution Science and WG3 Policy passages were consistently the weakest, because normative or governance-focused text is harder to anchor unambiguous multiple-choice questions to than quantitative or mechanistic passages. A more deliberate passage selection pass — favouring sections dense with quantitative findings — would improve generation quality before any curation is needed.',
    },
    {
      title: 'Blind, multi-annotator curation',
      body: 'The review was not blind to model identity. The reviewer could see which model generated each question, which may have introduced bias toward Claude\'s question style — partially explaining Claude\'s 46.9% acceptance rate versus 22–25% for others. A production benchmark would present questions without model attribution and compute inter-annotator agreement across at least two reviewers.',
    },
    {
      title: 'Held-out generation model',
      body: 'The cleanest fix for the self-advantage confound is to generate questions with a model that is not one of the evaluated models — removing self-familiarity as a factor entirely. Claude\'s 9.1pp self-advantage cannot be cleanly separated from genuine question quality in the current design.',
    },
  ],
}
...
function ClimateQACaseStudySection() {
  const [open, setOpen] = useState(false)
    ...
    <div className="mt-6 border-t border-slate-700/50 pt-6">
      ...
      {open && (
        <div className="mt-6 space-y-8 text-sm leading-relaxed text-slate-400">
          ...
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-teal-400">Introduction</p>
            <div className="space-y-3">
              {CLIMATE_QA_CASE_STUDY.intro.map((p, i) => <p key={i}>{p}</p>)}
            </div>
          </div>

          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-teal-400">Key Design Decisions</p>
            <div className="space-y-4">
              {CLIMATE_QA_CASE_STUDY.decisions.map(({ title, body }) => (
                <div key={title}>
                  <p className="mb-1 font-semibold text-slate-300">{title}</p>
                  <p>{body}</p>
                </div>
          ...
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-teal-400">What I&apos;d Do Next</p>
            <div className="space-y-4">
              {CLIMATE_QA_CASE_STUDY.nextSteps.map(({ title, body }) => (
                <div key={title}>
                  <p className="mb-1 font-semibold text-slate-300">{title}</p>
                  <p>{body}</p>
                </div>
          ...
          <div className="rounded-2xl border border-slate-700/60 bg-slate-800/50 p-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-teal-500/25 bg-teal-500/15">
                  <Globe className="h-5 w-5 text-teal-400" />
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white">{CLIMATE_QA.name}</h3>
                  <p className="text-xs text-slate-500">{CLIMATE_QA.year} · {CLIMATE_QA.type}</p>
                </div>
              </div>
            ...
            <p className="mt-5 text-sm leading-relaxed text-slate-300">{CLIMATE_QA.description}</p>

            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {CLIMATE_QA.highlights.map(({ icon: Icon, label, desc }) => (
                <div key={label} className="rounded-lg border border-slate-700/50 bg-slate-900/50 p-4">
                  <Icon className="mb-2 h-4 w-4 text-teal-400" />
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {CLIMATE_QA.tech.map(t => (
                <span key={t} className="rounded-full border border-teal-500/25 bg-teal-500/10 px-3 py-1 text-xs font-medium text-teal-300">
                  {t}
                </span>
              ))}
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href={CLIMATE_QA.githubUrl} target="_blank" rel="noopener noreferrer"
